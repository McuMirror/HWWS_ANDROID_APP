	...
	ui(new Ui::MainWindow)
{
	...
	qDebug()<< "ScreenWitdth: " << screenWidth;
	...
	if(settings.value("FirstUse", "").toBool() == false) //show DialogFirstUse if the app is started first time
	{
		while(settings.value("FirstUse","").toBool() == false)
		{
			...
			DFU->setFixedWidth(screenWidth);
			DFU->setFixedHeight(screenHeight);
...
void MainWindow::makeLayout(void)
{
	...
	ui->tableWidget_Warnings->setColumnWidth(0, (screenWidth - 40) * (3.0/5.0));
	ui->tableWidget_Warnings->setColumnWidth(1, (screenWidth - 40) * (2.0/7.0));

	qDebug() << "tableWidget_Warnings Width: " << ui->tableWidget_Warnings->width();
	//ui->tableWidget_Warnings->setColumnWidth(0, 310);
	...
	ui->comboBox->setMaximumHeight(screenHeight/25);
	...
	ui->label_Pitch_text->setText("Steigung:");
	ui->label_Pitch_text->setFont(defaultHeight);
	...
	ui->label_Pitch->setText("-");
	ui->label_Pitch->setFont(defaultHeight);
	...
	ui->tableWidget_attention->setColumnWidth(0, ui->tableWidget_attention->width() * (3.0/4.0));
	ui->tableWidget_attention->setColumnWidth(1, ui->tableWidget_attention->width() * (2.0/7.0));
...
void MainWindow::readFromQSettings(void)
{
	...
	ui->spinBox_attentionDiff->setValue(safety_diff);
...
void MainWindow::on_pushButton_add_clicked(void)   //add new warning
{
	...
	if(A.exec() == QDialog::Accepted)
	{
		...
		ui->tableWidget_Warnings->setRowHeight(rows, screenHeight/20);
		//ui->tableWidget_Warnings->columnWidth(340);
		ui->tableWidget_Warnings->setColumnWidth(0, ui->tableWidget_Warnings->width() * (3.0/4.0));
		ui->tableWidget_Warnings->setColumnWidth(1, ui->tableWidget_Warnings->width() * (2.0/7.0));
...
void MainWindow::updateGraphData (void)    //update the graph and output
{
	...
	if(downloadFail == false)     //if there is a download failure the prognose and the last value would be false
	{
		...
		ui->label_Pitch->setText("ca. " + QString::number(((steigung_C + steigung_L)/2.0) * 60.0, 'g', 2) + " cm/h");
...
void MainWindow::on_pushButton_login_clicked(void)
{
	if(LoginState == false)
	{
		...
		pal.setColor(QPalette::Background, Qt::green);
		L->setPalette(pal);
		L->setFixedWidth(screenWidth);
		L->setGeometry(0, 0,screenWidth, screenHeight/3);
...
void MainWindow::writeTableWidgetWarnings(void)
{
	...
	for(int i=0; i<Warnings.size(); i++)
	{
		...
		ui->tableWidget_Warnings->setColumnCount(2);
		ui->tableWidget_Warnings->setRowHeight(rows, screenHeight/ 20);
		//ui->tableWidget_Warnings->setColumnWidth(0, (screenWidth - 40) * (3.0/4.0));
		//ui->tableWidget_Warnings->setColumnWidth(1, (screenWidth - 40) * (1.0/7.0));
...
void MainWindow::checkWarningAttentions(void)
{
	...
	if((downloadFail == false) && (LoginState == true)) //if there was no error while downloading all warnings will be checked
	{
		for(int i=0; i<Warnings.size(); i++)
		{
			if(Warnings.at(i)->getStateWarningAtOverUnderheight() == false)
			{
				if((Warnings.at(i)->getHeight() <= lastValue_C + safety_diff ||
					Warnings.at(i)->getHeight() <= lastValue_L + safety_diff) && Warnings.at(i)->getTriggerState() == false)
				{
					count++;
					...
					ui->tableWidget_attention->setRowHeight(rows, screenHeight/20);
					...
					double k=0;
					if(((steigung_C_5min > 0) && (steigung_L_5min >0)) ||
							((steigung_C_5min < 0) && (steigung_L_5min < 0)) ||
							((steigung_C_5min == 0) && (steigung_L_5min == 0)))
					{
...
void MainWindow::on_spinBox_attentionDiff_editingFinished()
{
	safety_diff = ui->spinBox_attentionDiff->value();
	save_safty_diff();
}
